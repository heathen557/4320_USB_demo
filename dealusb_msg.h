...
public:
    ...
    int r,g,b,rgb;
    float integrateNum_ ;      //积分次数


    //文件存储相关
    QString tofPeakToSave_string;   //存储文件所需的tof和peak;
    QString tofPeakNum[16385];      //两者配合使用   ,前面加了一个标签
    ...
    int calibration_mean_num;     //校正时 取的平均的个数start_autoCalibration_slot
    ...
    vector<float> vec_mean_y[16384];
    QString y_offset[16384];


    /*******kalman 滤波算法相关*****************/
    float x_k[16384];     //状态 :状态初值设为第一个观测值
    float p_k[16384];     //状态协方差序列
    float K[16384];       // 卡尔曼增益
    int kalman_F;         // 状态转移矩阵
    int kalman_H;         // 观测矩阵
    ...
    //peak卡尔曼滤波
    float peak_x_k[16384];
    float peak_p_k[16384];
    float peak_K[16384];
    ...
    int last_id;    //上一帧曝光时间
    int last_package_num;     //上一包的标号
    int frame_index;          //第几帧数据 0 1 2 3 4 。。。。
    int hongPixel_line;       //0 1 2 3 4 ... 29  第几行的宏像素
    int pixel_index;          //0 1 2 3 ; 接收的第几个pixel的数据
    int TDC_first_index;      // 0 1 2 3   数据包中第一个TDC的编号
    bool isFirst_frame;       //是否是第一帧的标识  ，默认为True
    QString rawData_row_up[160];
    ...
    //中值滤波算法相关
    float src_tof[256][64];
    float src_peak[256][64];
    void tof_filter();
    void tof_filter_2();   //同步到点云信息



    /************RawData  MA 算法相关 *************************/
    QVector<double> historgramVec_MA;    //存储直方图的数据   支持最大4096
    int histogram_row;
    int histogram_col;
...
signals:
    ...
    /***************RAWDATA 相关***********************/
    void create_rawData_Dir_signal(int);
    void start_saveRawDataUp_signal(int,int,QStringList);   //帧数，行（0-120）,160个数据的QStringList
    void start_saveRawDataDown_signal(int,int,QStringList);   //帧数，行（0-120）,160个数据的QStringList

    /**************RawData  MA 相关****************************/
    void toShowHistogram_signal(QVector<double> ,int );
    void currentFrame_signal(int);    //当前帧
